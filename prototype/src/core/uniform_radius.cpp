...
template <int DIM>
using RSAMPIUniformRadiusGeneratorTmpl = RSAMPIUniformRadiusGenerator<DIM>;

ONIKA_AUTORUN_INIT(uniform_radius_generator) {
  OperatorNodeFactory::instance()->register_factory("uniform_radius_generator",
                                                    make_rsa_mpi_operator<RSAMPIUniformRadiusGeneratorTmpl>);
}
}  // namespace rsa_mpi
