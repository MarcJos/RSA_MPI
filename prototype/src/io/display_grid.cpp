...
namespace rsa_mpi
{
  ...
  using namespace onika::scg;
	...
	template <int DIM> class 
		RSAMPIDisplayGrid : public OperatorNode
	{
	...
	template<int DIM> using RSAMPIDisplayGridTmpl = RSAMPIDisplayGrid<DIM>;

	ONIKA_AUTORUN_INIT(rsa_mpi_grid) 
	{ 
		OperatorNodeFactory::instance()->register_factory("rsa_mpi_display_grid", make_rsa_mpi_operator<RSAMPIDisplayGridTmpl>); 
	} 
}
